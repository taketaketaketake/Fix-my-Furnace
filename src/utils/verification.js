...
export function generateExpirationTime(minutes = 10) {
  const expires = new Date();
  expires.setMinutes(expires.getMinutes() + minutes);
  return expires.toISOString();
}

export async function createVerificationRecord(supabase, formData, phoneNumber) {
...
export async function verifyPhoneToken(supabase, token) {
...
export async function incrementVerificationAttempts(supabase, phoneNumber) {
  const { data, error } = await supabase
    .from('form_submissions')
    .update({ 
...
export async function canResendVerification(supabase, phoneNumber, maxAttempts = 3) {
  const { data, error } = await supabase
    .from('form_submissions')
...
export async function cleanupExpiredVerifications(supabase, hoursOld = 24) {
